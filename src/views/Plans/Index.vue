...
<template>
  <DefaultLayout>
    ...
    <div v-if="UserService.getUserPermissions().includePlan" class="flex justify-end mt-6">
    ...
    <DataTableMain>
      <DataTable v-model:filters="filters" :value="plans" stripedRows paginator @row-edit-init="onEditing"
        ...
        <Column v-if="UserService.getUserPermissions().editPlan" header="Editar" :rowEditor="true" style="width: 5%; min-width: 8rem"></Column>
        <Column v-if="UserService.getUserPermissions().deletePlan" style="width: 5%; min-width: 8rem" header="Excluir">
          <template #body="{ data }">
